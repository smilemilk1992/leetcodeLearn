...
hashmap的扩容因子为什么是0.75-https://www.jianshu.com/p/5730bff593ed


##############################
1.7 put会出现死循环 头插法
在对table进行扩容到newTable后，需要将原来数据转移到newTable中，
这里可以看出在转移元素的过程中，使用的是头插法，也就是链表的顺序会翻转，这里也是形成死循环的关键点。
1.8 put会出现数据覆盖 尾插法
线程A进入后还未进行数据插入时挂起，而线程B正常执行，从而正常插入数据，然后线程A获取CPU时间片，
